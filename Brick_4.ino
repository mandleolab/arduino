...
if (ballX >= SCREEN_WIDTH - BALL_SIZE) {
  ballX = SCREEN_WIDTH - BALL_SIZE;  // 위치 보정
  ballSpeedX *= -1;
}
if (ballY <= 0) {
  ballY = 0;               // 위치 보정
...
if (ballY + BALL_SIZE >= paddleY && ballY + BALL_SIZE <= paddleY + PADDLE_HEIGHT &&
  ...
  ballSpeedY = -abs(ballSpeedY);  // 공을 위쪽으로 튕김

  // 공이 닿은 패들 위치에 따라 좌우 방향도 약간 변경 (반사각 효과)
